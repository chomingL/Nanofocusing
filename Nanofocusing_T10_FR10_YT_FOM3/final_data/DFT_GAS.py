...
def Et_empty_fun(Ex_emp,Ey_emp,Ez_emp):
    ...
    ET_y = 0
    ET_z = 0
    ET_x_length = Ex_emp.shape[1]
    ET_y_length = Ey_emp.shape[1]
    ET_z_length = Ez_emp.shape[1]
    for k in range(0,ET_x_length):
        ET_x = ET_x + npa.abs(Ex_emp[:,k]) ** 2
    for k in range(0,ET_y_length):
        ET_y = ET_y + npa.abs(Ey_emp[:,k]) ** 2
    for k in range(0,ET_z_length):
        ET_z = ET_z + npa.abs(Ez_emp[:,k]) ** 2
    ET = ( (npa.mean(ET_x)) + (npa.mean(ET_y)) + (npa.mean(ET_z)) ) ** (1/2)  
    return ET

ET_empty = Et_empty_fun(Ex_emp,Ey_emp,Ez_emp)
np.save(f'FOM_ET_empty.npy', ET_empty)
##########################################################################################################################
...
dft_fields_CD_pos = opt.sim.add_dft_fields([mp.Ex,mp.Ey,mp.Ez],
                                fcen,0,1,
                                center=mp.Vector3(0,0,design_region_z_height+0.01),
                                size=mp.Vector3(Sx,Sy,0),
                                yee_grid=True 
                                )
